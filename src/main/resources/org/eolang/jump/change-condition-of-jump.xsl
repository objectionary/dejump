...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" id="change-condition-of-jump" version="2.0">
    ...
    <xsl:template match='o[@base=".if" and o[2][@base=".forward"]]'>
        <xsl:element name="o">
            ...
            <xsl:attribute name="which"><xsl:value-of select="o[2]/o[1]/@base"/></xsl:attribute>
            <xsl:attribute name="tt"><xsl:text>f</xsl:text></xsl:attribute>
    ...
    <xsl:template match='o[@base=".if" and o[2][@base=".backward"]]'>
        <xsl:element name="o">
            <xsl:attribute name="base"><xsl:value-of select="@base"/></xsl:attribute>
            <xsl:attribute name="which"><xsl:value-of select="o[2]/o[1]/@base"/></xsl:attribute>
            <xsl:attribute name="tt"><xsl:text>b</xsl:text></xsl:attribute>
            <xsl:copy-of select="o[1]"/>
            <xsl:element name="o">
                <xsl:attribute name="base">.write</xsl:attribute>
                <xsl:element name="o">
