...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match='o[ends-with(@base,"goto")]'>
        ...
        <xsl:element name="o">
            <xsl:attribute name="base"><xsl:text>.while</xsl:text></xsl:attribute>
            <xsl:choose>
                <xsl:when test='$current//o[@rem=$curName]'>
                    <xsl:element name="o">
                        <xsl:attribute name="base"><xsl:text>.or</xsl:text></xsl:attribute>
                        <xsl:for-each select='$current//o[@rem=$curName]'>
                            <xsl:element name="o">
                                <xsl:attribute name="base"><xsl:text>.eq</xsl:text></xsl:attribute>
                                <xsl:element name="o">
                                    <xsl:attribute name="base"><xsl:value-of select="@fl"/></xsl:attribute>
                                </xsl:element>
                                <xsl:element name="o">
                    ...
                    <xsl:element name="o">
                        ...
                        <xsl:element name="o">
                            <xsl:attribute name="name"><xsl:text>i</xsl:text></xsl:attribute>
                        </xsl:element>
                        <xsl:element name="o">
                ...
                <xsl:otherwise>
                    ...
                    <xsl:element name="o">
                        <xsl:attribute name="base"><xsl:text>.eq</xsl:text></xsl:attribute>
                        <xsl:element name="o">
                            <xsl:attribute name="base"><xsl:value-of select="$tempFlag"/></xsl:attribute>
                        </xsl:element>
                        <xsl:element name="o">
                            <xsl:attribute name="base"><xsl:text>org.eolang.int</xsl:text></xsl:attribute>
                            <xsl:attribute name="data"><xsl:text>int</xsl:text></xsl:attribute>
                    ...
                    <xsl:element name="o">
                        ...
                        <xsl:element name="o">
                            ...
                            <xsl:element name="o">
                                ...
                                <xsl:element name="o">
                                    <xsl:attribute name="base"><xsl:value-of select="$tempFlag"/></xsl:attribute>
                                    <xsl:attribute name="temp"><xsl:value-of select="$tempFlag"/></xsl:attribute>
                                </xsl:element>
                                <xsl:element name="o">
                                    <xsl:attribute name="base"><xsl:text>org.eolang.int</xsl:text></xsl:attribute>
                                    <xsl:attribute name="data"><xsl:text>int</xsl:text></xsl:attribute>
